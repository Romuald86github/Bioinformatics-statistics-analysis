...
# Perform PERMANOVA for beta diversity comparison between locations
# Check if all location values are unique
if len(metadata['Location'].unique()) == len(metadata['Location']):
    print("All locations contain unique groups, PERMANOVA cannot be performed.")
else:
    # Run PERMANOVA to test for significant differences in beta diversity between locations
    permanova_results = permanova(bray_curtis_distances_no_nan, metadata['Location'], permutations=99999)
    
    print("PERMANOVA results for locations:")
